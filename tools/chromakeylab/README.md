...
6. **Despill** for magenta fringe reduction at edge pixels.
...
- `Despill x100`: magenta fringe reduction (the despill cell is tuned for a
  magenta key; for other key colours the residual QA cell in the notebook
  catches what's left, and `Dilate k` is the usual response).
