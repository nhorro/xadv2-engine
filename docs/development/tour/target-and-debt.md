...
Working reference for what XADV2 *should* be, how far the tree is from that,
and how to move without stranding Fuera de Cuadro.

Historical pages under `docs/development/design/` are an archive of
...
This repository now treats `docs/development/design/` as frozen history
([note](../history/README.md)). The as-built tour is `docs/development/tour/`.
Do not rewrite 01–06 in place.

`scumm-command-flow.md` points at `RoomCommandProcessor`. Do not extract a
third dispatcher.
...
Tour, not the archive: [core](01-core-abstractions.md), then
[P&C kit](07-point-and-click.md). Acceptance: a new developer can trace
