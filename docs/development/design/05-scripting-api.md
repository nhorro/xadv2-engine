...
| `open_cutscene(scene_id)` | `Cutscene` scene id | — | Push a declarative cutscene over the live room. Its manifest entry should use `on_finish: POP` so completion or ESC returns to the preserved room. |
