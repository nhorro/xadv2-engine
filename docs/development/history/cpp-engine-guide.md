...
This guide describes how current engine C++ is written. The
[as-built architecture tour](../tour/index.md) describes the runtime concepts;
the code and tests settle details when documentation is incomplete.

This guide applies to C++ code under `lib/`, `examples/`, `tests/`, `android/`,
and `packaging/`.
...
├── examples/                    Focused executable feature contracts
├── tests/                       Automated runtime tests
├── android/                     Android build and launcher
├── packaging/                   External-consumer smoke project
├── docs/authoring/              Current Lua and YAML API
├── docs/development/tour/       Current implementation tour
├── docs/development/plans/      Explicitly non-current proposals
├── docs/development/history/    Superseded design material
...
Layers: `core`, `geom`, `gfx`, `pnc`. Dependency direction is one-way; see the
[architecture tour](../tour/index.md).
...
Public names in the current authoring API are compatibility contracts. Confirm
C++ details in public headers and tests before relying on prose.
